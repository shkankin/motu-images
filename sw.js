...
//     against live main before each change.
//   • REL-01 (CRITICAL) photo reads no longer depend on CSP AT ALL. v7.84
//     shipped the connect-src blob:/data: fix and it NEVER REACHED
//     PRODUCTION: that release's sw.js and data.js deployed, motu-vault.html
//     did not, so the changelog described a fix users never received while
//     Prune stayed armed to delete healthy photos. The policy is fixed again
//     here, but photos.js v7.87 also removes the dependency: readPhotoBlob()
//     takes OPFS bytes straight off getFileHandle().getFile() and decodes
//     data: URLs in-process. _readable, getAllAsBlobs, exportAllAsDataURLs,
//     importPhotos and migrateLegacyLS no longer call fetch(). One meta tag
//     is no longer a single point of failure for the user's photo library.
//   • BUG-01 (HIGH) grid status button. patchFigRow set an inline onclick,
//     blocked by script-src since v7.00, and left data-action stale — so the
//     second tap on a newly owned card called setStatus(id,'owned') again,
//     which is treated as a repeat and toggles ownership OFF. Now sets
//     dataset.action/dataset.figId. lint_handlers.mjs gained a check for
//     runtime inline-handler assignment, narrowed to the STRING forms only
//     (el.onclick = fn is legal under CSP; toastUndo/toastAction rely on it).
//   • PERF-01 (HIGH) main-theme.mp3 (3.8 MB) out of the versioned SHELL —
//     ~45 releases since July meant ~170 MB re-downloaded per active user.
//     NOTE ui-sheets.js sets a RELATIVE src, so the existing .mp3 branch
//     (gated on raw.githubusercontent.com) never saw it; a same-origin media
//     branch into the unversioned IMG_CACHE was added, otherwise removing it
//     from SHELL would have changed nothing.
//   • A11Y-01 (HIGH) user-scalable=no removed — pinch zoom works again.
//   • BUG-07 SHELL asked for 'masters_logo.png'; the file is in images/, so
//     install 404'd silently and the logo was missing offline.
//   • BUG-02 appConfirm/appPromptText hard-coded #fff on var(--acc); in the
//     He-Man theme that is 1.48:1. Now var(--btn-t), which every theme sets.
//   • PERF-02 refinements: cache:'no-cache' (cheap 304) instead of 'reload'
//     (full re-transfer), and ignoreSearch on navigations so ?action=/?m=
//     links resolve from cache offline.
//   • REL-02 af411-paths.json was generated by five scheduled syncs and
//     staged by none — the v7.85 AF411 deep-link fix has been inert in
//     production. The workflow now git-adds it. Added the .gitignore
//     sync_af411.py has referenced since v1.5 (DOC-05) and untracked
//     scripts/__pycache__.
//   • PROC-01 scripts/check_version_stamps.mjs asserts every shell file's
//     stamp matches CACHE in sw.js, and is wired into lint.yml. This is the
//     check that would have caught REL-01: check_deployable proves a file
//     made it into the zip, not that it reached production.
//   • Docs: DOC-01 (README called desktop.html read-only; it writes),
//     DOC-03 (backend README overstated CORS), DOC-04 (manifest shortcut).
...
function networkFirstWithDeadline(e, cacheName) {
  const req = e.request;
  // v7.87: 'no-cache' (revalidate, allow a cheap 304) instead of 'reload'
  // (always re-transfer the full body). Same freshness, far less data.
  ...
  // v7.87: ignoreSearch so ?action=/?m= navigations match the cached shell
  // offline instead of missing on the query string.
  const lookup = () => caches.match(req, { ignoreSearch: req.mode === 'navigate' });
  return lookup().then(cached => {
    if (!cached) return net.catch(() => lookup());
...
const SHELL = [
  ...
  'manifest.json',
  'images/masters_logo.png',   // v7.87 BUG-07: was 'masters_logo.png'; the file has always lived in images/, so this SHELL entry 404'd on every install and the error was swallowed — the logo was missing offline until first viewed online.
  // v7.87 PERF-01: main-theme.mp3 (3.8 MB) REMOVED from the versioned shell.
  // It was fetched with cache:'reload' on every install and dropped with the
  // old cache on activate, so each active user re-downloaded it on all 45+
  // sw.js releases since July — roughly 170 MB of mobile data for a track
  // that plays in one sheet. It now loads on demand and lands in the
  // unversioned media cache by the SAME-ORIGIN branch added below. NOTE the
  // existing .mp3 branch is gated on raw.githubusercontent.com, and
  // ui-sheets.js sets a RELATIVE src ('main-theme.mp3'), so that branch
  // never sees it — without the same-origin route it would land straight
  // back in the versioned CACHE and be re-downloaded every release anyway.
  // Do NOT put it back in SHELL.
  'vault.css',
  'js/app.js',
  'js/state.js',
  'js/idb-store.js',
...
self.addEventListener('fetch', e => {
  ...
  // v7.87 PERF-01: same-origin media (the About-sheet theme track and the
  // egg sounds) — cache-first into the UNVERSIONED media cache so it
  // survives activate(), which deletes every cache except CACHE and
  // IMG_CACHE. Must sit ABOVE the catch-all network-first branch.
  if (url.origin === location.origin && /\.(mp3|ogg|m4a)$/.test(url.pathname)) {
    e.respondWith(
      caches.match(e.request).then(cached => cached || fetch(e.request).then(res => {
        if (res.ok) {
          const clone = res.clone();
          e.waitUntil(caches.open(IMG_CACHE).then(c => c.put(e.request, clone)));
        }
        return res;
      }))
    );
